-- Fix trips "new row violates row-level security" on cloud sync.
-- Run in Supabase SQL Editor after 001–003 (safe to re-run).

-- Reclaim trips saved with old anonymous device owner ids (not real auth users).
...
where t.id = tm.trip_id
  ...
  and t.owner_id not in (select id from auth.users);

-- If a trip has no member row yet but owner_id is orphaned, fix manually:
-- update public.trips set owner_id = 'YOUR_USER_UUID' where id = 'YOUR_TRIP_UUID';
